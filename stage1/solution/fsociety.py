...
from pipeline.submission_pipe import *
import argparse
...
def sub():
    sub_path=input("Enter the submission directory:")
    modelpath=input("Enter model path to be evaluated:")
    return submission_pipe(sub_path,modelpath)

def test():
    test_path=input("Enter the test filename:")
...
def main(input_):
    while True:
        text = input("Press [Enter] to start  [h] for help !!")
        if text == "":
...
mode=['train','validate','test','submit']
